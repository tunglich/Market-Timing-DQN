"""Train 5 SB3 agents with CapAnchoredPortfolioEnv using FinRL default 8 indicators.

Variant E: Variant A's feature set + Variant D's cap-anchored method.

Features (state):
    Indicators 8x50: macd, boll_ub, boll_lb, rsi_30, cci_30, dx_30,
                     close_30_sma, close_60_sma
    Cov_matrix 50x50: 252-day rolling
    State shape: (58, 50)

Action (cap-anchored):
    weights = softmax(log(cap_w) + action),  action in Box(±3)

Reward (shaping):
    portfolio_value * (1 - lambda_dev * Sigma((w - cap_w)^2))   lambda_dev = 0.5

Saves models to tw50_capweighted_finrl_trained_models/agent_<name>.zip
TB logs to tw50_capweighted_finrl_results/<name>.
"""
...
RESULTS_DIR = str(FINRL_ROOT / "tw50_capweighted_finrl_results")
...
print("=== Loading tw50_capweighted_finrl_train.pkl ===")
...
print(f"Stock dim={stock_dim}  Indicators={INDICATORS}  "
      f"State shape=({state_space + len(INDICATORS)}, {state_space})")
