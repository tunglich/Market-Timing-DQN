"""TW50 cap-weighted prep: FinRL 8 indicators + DES (from all_60) + 252-day cov.

Source: data/<tic>_all_60.csv  (50 tickers from data/tw50_2023-12-29.csv)
        Columns: <DATE>,<DES>,<OPEN>,<HIGH>,<LOW>,<CLOSE>,<VOLUME>
        Pre-IPO zero OHLC backfilled with first non-zero close.

Output:
  tw50_capweighted_finrl_des60_train.pkl  (2006-01-09 ~ 2023-12-29 ; first 252d skipped)
  tw50_capweighted_finrl_des60_trade.pkl  (2024-01-02 ~ 2026-03-31)
...
def load_one(ticker: str) -> pd.DataFrame:
    p = DATA_DIR / f"{ticker}_all_60.csv"
    df = pd.read_csv(p)
...
print(f"\n=== Reading {len(tickers)} *_all_60.csv files ===")
...
train.to_pickle("tw50_capweighted_finrl_des60_train.pkl")
trade.to_pickle("tw50_capweighted_finrl_des60_trade.pkl")
print("\nSaved: tw50_capweighted_finrl_des60_train.pkl, tw50_capweighted_finrl_des60_trade.pkl")
