"""Backtest 5 cap-anchored agents (FinRL 8 + DES from all_65) on TW50.
...
OUT_DIR = FINRL_ROOT / "results/capweighted_finrl_des65"
OUT_DIR.mkdir(parents=True, exist_ok=True)
TRAINED_MODEL_DIR = str(FINRL_ROOT / "tw50_capweighted_finrl_des65_trained_models")
PRIOR_BT = FINRL_ROOT / "results/capweighted_finrl_des75" / "backtest_results.csv"
...
print("=== Loading data ===")
train = pd.read_pickle("tw50_capweighted_finrl_des65_train.pkl")
trade = pd.read_pickle("tw50_capweighted_finrl_des65_trade.pkl")
    ...
    try:
        ...
        for col in ["a2c", "ppo", "ddpg", "td3", "sac"]:
            if col in prior.columns:
                combined[f"{col}_75"] = prior[col]
        print(f"\nLoaded all_75 overlay from {PRIOR_BT.name}")
    except Exception as e:
...
plot_cols = [c for c in combined.columns if not c.endswith("_75")]
...
plt.title(f"FinRL TW50 Cap-Anchored + FinRL 8 + DES (all_65, 60K timesteps)  "
...
summary = [
    "FinRL TW50 OOS Backtest -- Cap-Anchored + FinRL 8 indicators + DES (all_65)",
