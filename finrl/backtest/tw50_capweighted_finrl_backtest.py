"""Backtest 5 cap-anchored agents (Variant E: FinRL 8-indicators) on TW50.
...
Compares vs:
  ...
  - Variant D (DES+OHL features) results overlaid as *_D columns (optional)
"""
...
OUT_DIR = FINRL_ROOT / "results/capweighted_finrl"
OUT_DIR.mkdir(parents=True, exist_ok=True)
TRAINED_MODEL_DIR = str(FINRL_ROOT / "tw50_capweighted_finrl_trained_models")
PRIOR_BT = FINRL_ROOT / "backtest_tw50_capweighted_2024_20260331" / "backtest_results.csv"
...
print("=== Loading data ===")
train = pd.read_pickle("tw50_capweighted_finrl_train.pkl")
trade = pd.read_pickle("tw50_capweighted_finrl_trade.pkl")
...
print("\n=== Running DRL agents (cap-anchored, FinRL indicators) ===")
results = {}
weights_memory = {}
for name, model in agents.items():
    ...
    try:
        ...
        print(f"  {name.upper()}: final = ${float(df_dret['account_value'].iloc[-1]):,.0f}")
    except Exception as e:
        import traceback
...
print(f"CAP_BH final = ${float(cap_bh_value.iloc[-1]):,.0f}")
...
if PRIOR_BT.exists():
    try:
        prior = pd.read_csv(PRIOR_BT, index_col=0)
        for col in ["a2c", "ppo", "ddpg", "td3", "sac"]:
            if col in prior.columns:
                combined[f"{col}_D"] = prior[col]
        print(f"\nLoaded Variant D (DES+OHL) overlay from {PRIOR_BT.name}")
    except Exception as e:
        print(f"Prior overlay skipped: {e}")
...
plot_cols = [c for c in combined.columns if not c.endswith("_D")]
...
plt.title(f"FinRL TW50 Cap-Anchored + FinRL 8-indicators  (Variant E)  "
...
summary = [
    "FinRL TW50 OOS Backtest - Variant E (Cap-Anchored + FinRL 8-indicators)",
    ...
    f"Training reward: portfolio_value * (1 - lambda_dev * Sigma((w-cap)^2)),  lambda_dev=0.5",
    f"State: 252-day rolling cov_matrix (50x50) + indicators ({INDICATORS} 8x50)",
    f"Transaction cost: NOT MODELED (FinRL portfolio env limitation)",
    f"CAP_BH: cap-weighted buy-and-hold using 2023-12-29 weights from {TW50_LIST.name}",
    "",
