"""Backtest 5 cap-anchored agents (FinRL 8 + DES from all_60) on TW50.
...
OUT_DIR = FINRL_ROOT / "results/capweighted_finrl_des60"
OUT_DIR.mkdir(parents=True, exist_ok=True)
TRAINED_MODEL_DIR = str(FINRL_ROOT / "tw50_capweighted_finrl_des60_trained_models")
PRIOR_BT_75 = FINRL_ROOT / "results/capweighted_finrl_des75" / "backtest_results.csv"
PRIOR_BT_65 = FINRL_ROOT / "results/capweighted_finrl_des65" / "backtest_results.csv"
...
print("=== Loading data ===")
train = pd.read_pickle("tw50_capweighted_finrl_des60_train.pkl")
trade = pd.read_pickle("tw50_capweighted_finrl_des60_trade.pkl")
...
for prior_path, suffix in [(PRIOR_BT_75, "75"), (PRIOR_BT_65, "65")]:
    if prior_path.exists():
        try:
            prior = pd.read_csv(prior_path, index_col=0)
            for col in ["a2c", "ppo", "ddpg", "td3", "sac"]:
                if col in prior.columns:
                    combined[f"{col}_{suffix}"] = prior[col]
            print(f"Loaded all_{suffix} overlay from {prior_path.parent.name}")
        except Exception as e:
            print(f"all_{suffix} overlay skipped: {e}")
...
plot_cols = [c for c in combined.columns if not (c.endswith("_75") or c.endswith("_65"))]
...
plt.title(f"FinRL TW50 Cap-Anchored + FinRL 8 + DES (all_60, 60K timesteps)  "
...
summary = [
    "FinRL TW50 OOS Backtest -- Cap-Anchored + FinRL 8 indicators + DES (all_60)",
